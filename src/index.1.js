...
const Pdf = Loadable({
  loader: () => import('./Component/Pdf'),
  loading: Loading,
  timeout: 10000
});
const VideoPlay = Loadable({
  loader: () => import('./Component/VideoPlay'),
  loading: Loading,
  timeout: 10000
});
...
const Disease = Loadable({
  loader: () => import('./Component/Disease'),
  loading: Loading,
  timeout: 10000
});
...
window.openPages =async()=>{
  let pageId = localStorage.getItem('pageIdnew');
  let userid = localStorage.getItem('userId');
  let title = localStorage.getItem('titlenew');
...
//注入config
window.getTestWxShareConfig = (url=window.location.href) =>{
  ...
  //百度监测
    ...
    getdata = ()=>{
      ...
      if(code){
        ...
        +'&redirect_uri=' + encodeURIComponent(window.location.href) +'&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect'
      }else{
    ...
    window.wxshare=(a,b)=>{
      ...
      //获取分享出来的链接的params {userId，suserid，ouserid}
        ...
        let userid = localStorage.getItem("userId");//用户id
          ...
          if(!userid){//分享出来的链接没有缓存userId，此时的用户id为链接上的suerid
        ...
        let getuser = params.userid;
            userid  = !getuser?userid:ouserid;//如果二次分享取url上的ouserid
        let suserid= !getuser?userid:gtuserid;//如果没有上级gtuserid，就是用当前用户id

  
        let baseurls = serverUrl+`?ouserid=${userid}&suserid=${suserid}`;//分享时需要带上的一个url
        ...
        let urlsplit =  urlbase.indexOf('?p13no')!= -1 ?urlbase.split('?p13no')[0]:urlbase.split('?backurl')[0];
            ...
            urlsplit = urlsplit.replace('?from=singlemessage','');//如果是二次分享就截去 多余的这个参数

        //拼接分享链接
            a.link=  urlsplit + `${encodeURIComponent('?backurl='+baseurls)}`;
            b.link=  urlsplit + `${encodeURIComponent('?backurl='+baseurls)}`;
        ...
        return (
            <Router>
                <div>
                    ...
                    <Route exact path='/Iframe/:doctype/:docId/:url' component={Iframe}/>
                </div>
            </Router>
        );
    }
